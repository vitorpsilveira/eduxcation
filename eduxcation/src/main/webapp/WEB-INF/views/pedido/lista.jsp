		...
		<div class="row">
			...
			<div class="input-field col s3 offset-s3">
				<a class="btn-floating btn-large waves-effect waves-light red right"
					title="novo" href="${s:mvcUrl('novoPedidoCompraUrl').build()}">
							...
							<f:form method="GET" modelAttribute="pedidoFiltroForm"
								...
								<div class="row">
									<div class="input-field col s4">
										<label> <f:radiobutton path="tipoFiltro" value="RS" />
								...
								<div class="row">
									<div class="input-field col s4">
										<label> <f:radiobutton path="tipoFiltro" value="ST" />
											<span>Status</span>
										</label>
									</div>
									<div class="input-field col s8">
								...
								<div class="row">
									<div class="input-field col s4">
										<label> <f:radiobutton path="tipoFiltro" value="DT" />
									...
									<div class="input-field col s4">
										<label class="active" for="entregaFinal">Entrega
											(Final)</label>
					...
					<table>
						...
						<tbody>
							<c:forEach items="${listaPaginada.content}" var="registro">
								<tr>
									<fmt:parseDate value="${registro.dataEntrega}"
										pattern="yyyy-MM-dd" var="dataModificada" type="date" />
									<td>${registro.id}</td>
									<td>${registro.fornecedor.razaoSocial}</td>
									<td><fmt:formatDate value="${dataModificada}"
											pattern="dd/MM/yyyy" /></td>
									<td>${registro.condicaoPagamento.displayValue}</td>
									<td><fmt:formatNumber value="${registro.valorTotal}"
											type="currency" /></td>
									<td>${registro.status.displayValue}</td>
									<td class="center-align">
										<c:choose>
											<c:when test ="${registro.status == 'EM_DIGITACAO'}">
												<a class="btn-small green" title="alterar"
												href="${s:mvcUrl('alterarPedidoCompraUrl').arg(0, registro.id).build()}"><i
												class="material-icons">edit</i></a>
											</c:when>
											<c:when test ="${registro.status == 'CANCELADO'}">
										...
										href="${s:mvcUrl('detalharPedidoUrl').arg(0, registro.id).build()}"><i
											class="material-icons">more_horiz</i></a> <c:if
											test="${registro.status == 'EM_DIGITACAO'}">
											<button class="btn-small red modal-excluir" title="excluir"
												type="button" data-target="modalExcluir"
												data-descr="${registro.id}">
												<i class="material-icons">delete</i>
												<f:form
													action="${s:mvcUrl('excluirPedidoCompraUrl').arg(0, registro.id).build()}"
													method="post">
												</f:form>
											</button>
										</c:if> <a
										href="${s:mvcUrl('geraPdfPedidoUrl').arg(0, registro.id).build()}"
										target="__blank" class="btn-small deep-purple"
										title="imprimir" type="button"> <i class="material-icons">print</i>
									</a></td>
								</tr>
							</c:forEach>
						</tbody>
